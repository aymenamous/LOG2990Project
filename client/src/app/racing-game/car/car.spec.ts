...
describe("Car", () => {
    ...
    it("collision force should dispel every frame", () => {
      car.setCollisionForce(new Vector3(5, 0, 0));
      const initialCollisionForce: Vector3 = car.getCollisionForce().clone();
      car.update(MS_BETWEEN_FRAMES);
      expect(car.getCollisionForce().x).toBeLessThan(initialCollisionForce.x);
  });
